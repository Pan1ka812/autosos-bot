...
COMPANY_NAME = os.getenv("COMPANY_NAME", "AutoSos")
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    caption_text = (
        "<b>AutoSos</b> — ваш надійний помічник на дорозі 🚗\n\n"
        "Застрягли? Потрібна евакуація або перевезення? Ми поруч!\n\n"
        "Що ми робимо:\n"
        "🚛 Евакуатор\n"
        "📦 Вантажні перевезення\n"
        "🏗 Гідравлічна платформа\n"
...
async def help_command(message: Message):
    ...
    help_text = (
        "<b>Як викликати AutoSos?</b>\n\n"
        "1️⃣ Натисніть <b>/order</b> — вкажіть ім'я та номер телефону.\n"
        "2️⃣ Оберіть послугу: Евакуатор, Вантажні перевезення, Гідравлічна платформа або Кран-маніпулятор.\n"
        "3️⃣ Вкажіть авто або вантаж, марку/модель, габарити та вагу (якщо потрібно).\n"
        "4️⃣ Терміново або на конкретний час — обирайте зручний варіант.\n"
        "5️⃣ Вкажіть адреси — текстом або через карту прямо в боті.\n"
        "6️⃣ За потреби додайте телефони на адресах та фото.\n"
        "7️⃣ Оберіть спосіб оплати: <b>Готівка</b> або <b>БН</b>.\n"
        "8️⃣ Перевірте заявку та натисніть <b>✅ Підтвердити</b>.\n\n"
        "Диспетчер отримає заявку і зв'яжеться з вами найближчим часом.\n\n"
        "👤 Профіль та історія замовлень: <b>/profile</b>\n\n"
        "Є питання? Зверніться до підтримки через профіль — ми завжди на зв'язку."
    )
...
async def process_photo(message: Message, state: FSMContext):
    ...
    if len(photo_ids) >= 5:
        await message.answer("Максимум 5 фото. Натисніть «✅ Готово» щоб продовжити.")
        return
    ...
    if len(photo_ids) >= 5:
        await message.answer("Досягнуто максимум 5 фото.")
        await ask_for_payer_with_price(message, state)
    else:
        await message.answer(
            f"📷 Фото {len(photo_ids)}/5 додано. Можете надіслати ще або завершити.",
